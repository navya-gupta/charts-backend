...
async def upload_csv(file: UploadFile, name: str = Form(...), university: str = Form(...), designation: str = Form(...), machine_used: str = Form(...)):
    print("KK1")
    global upload_paths
    try:
        # Read CSV content
        ...
        file_name = f"{name.replace(' ', '_')}_{timestamp}.csv"
        ...
        cursor.execute(
            INSERT_USER_UPLOAD,
            (upload_id, name, university, designation, machine_used, csv_content, file_name, timestamp) 
        )
        conn.commit()
